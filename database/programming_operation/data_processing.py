''' this script is to check the dataprocesing method in database'''
import sqlite3
import csv
import os
...
def main(database_name, table_name, insert_file, update_file, delete_file):
    ...
    create_file('01_student_info_before_updation.csv', cur_val, table_name,\
                    list_header) # File creation before updation
    insert_value(cur_val, insert_file) # Inserting values
    create_file('02_student_info_after_inserting.csv', cur_val, table_name, list_header)
    update_value(cur_val, list_header, table_name, update_file) # Updating value
    create_file('03_student_info_after_updation.csv', cur_val, table_name, list_header)
    delete_value(cur_val, table_name, list_header, delete_file) # Deleting values
...
def insert_value(cur_val, input_insert_file):
    ...
    insert_file = [b for b in insert_line] # pylint: disable=unnecessary-comprehension
    for i in range(1, len(insert_file)):
        cur_val.execute(f"insert into student values \
            ({insert_file[i][0]}, \'{insert_file[i][1]}', {insert_file[i][2]}, \
                \'{insert_file[i][3]}', {insert_file[i][4]})")
...
def create_file(file_name, cur_val, table_name, list_header):
    ''' Creation of files'''
    if os.path.exists(f"result{os.path.sep}{file_name}"):
